...
class Microphysical_Derivatives(TestCase):
    ...
    def setUpClass(cls):
        ...
        ext = 20.0
        veff = 0.1
        rte_grid = shdom.grid.make_grid(0.05, 13, 0.05, 13, np.linspace(0.1,0.7,13))
        ...
        for zenith,azimuth,wavelength in zip(sensor_zenith_list,sensor_azimuth_list,wavelengths):
            Sensordict.add_sensor('MISR', shdom.sensor.orthographic_projection(
                                                                wavelength,
        ...
        for wavelength in wavelengths:
            ...
            solver = shdom.solver.RTE(
                            ...
                            surface=shdom.surface.ocean_unpolarized(surface_wind_speed=10.0, pigmentation=0.0),
                            num_stokes=1,
                            name=None
                            )
...
class Verify_Jacobian(TestCase):
    ...
    def setUpClass(cls):
        ...
        veff = 0.1
        reff=10.0
        rte_grid = shdom.grid.make_grid(0.05, 3, 0.05, 3, np.arange(0.1, 0.25, 0.05))
        grid_shape = (rte_grid.x.size, rte_grid.y.size, rte_grid.z.size)
        rte_grid['density'] = (['x','y','z'], np.ones(grid_shape))
        rte_grid['reff'] = (['x','y','z'], np.zeros(grid_shape) + reff)
        rte_grid['veff'] = (['x','y','z'] ,np.zeros(grid_shape) + veff)
