...
class Load_from_csv_test(TestCase):
    def test_load_from_csv(self, path=str(importlib.resources.files('at3d').joinpath('data/synthetic_cloud_fields/jpl_les/rico32x37x26.txt'))):
        scatterer = at3d.util.load_from_csv(path, density='lwc', origin=(0.0,0.0))

class Microphysics_load_from_csv_test(TestCase):
    def setUp(self):
        self.droplets = at3d.util.load_from_csv(str(importlib.resources.files('at3d').joinpath('data/synthetic_cloud_fields/jpl_les/rico32x37x26.txt')), density='lwc', origin=(0.0,0.0))
...
class Test_resample_onto_grid(TestCase):
    def setUp(self):
        scatterer = at3d.util.load_from_csv(str(importlib.resources.files('at3d').joinpath('data/synthetic_cloud_fields/jpl_les/rico32x37x26.txt')), density='lwc', origin=(0.0,0.0))
