...
def make_config(config_file_name,horizontal_boundary_conditions=1,
    ...
    shdom_parameters = {
        ...
        'solution_accuracy': {'default_value':solution_accuracy, 'description': '(SOLACC) solution accuracy - tolerance for solution criterion.'},
        'max_total_mb': {'default_value': max_total_mb, 'description': 'approximate maximum memory to use (MB for 4 byte reals)'},
        'adapt_grid_factor': {'default_value': adapt_grid_factor, 'description': '(ADAPT_GRID_FACTOR) ratio of total grid points to base grid points'},
        'num_sh_term_factor': {'default_value': num_sh_term_factor, 'description': '(NUM_SH_TERM_FACTOR) ratio of average number of spherical harmonic terms to total possible (NLM)'},
        'cell_to_point_ratio': {'default_value': cell_to_point_ratio, 'description': '(CELL_TO_POINT_RATIO) ratio of number of grid cells to grid points'},
        'high_order_radiance': {'default_value': high_order_radiance, 'description': 'True to keep the high order radiance field in memory for diagnostic purposes.'},
        'ip_flag':{'default_value': ip_flag, 'description': ['(IPFLAG) Bit flags for independent pixel mode: 0 for 3D, 1 for independent (2D) scans in X, 2 for 2D scans in Y (X-Z planes)','3 for indepedent pixels (i.e. bit 0 for X and bit 1 for Y). Bit 2 of IPFLAG means do the direct beam in 3D,',' e.g. IPFLAG=7 means 3D direct beam but IP diffuse radiative transfer.']}
    }
