...
def parallel_gradient(solvers, rte_sensors, sensor_mapping, forward_sensors, gradient_fun=grad_l2,
    ...
    if mpi_comm is not None:
        ...
        if n_jobs == 1:
            out = [gradient_fun(solvers[key],rte_sensors[key],1, sensor_mapping[key],
                                                     forward_sensors, **kwargs) for key in solvers.keys()]
        else:
            #decide on the division of n_jobs among solvers based on total number of rays.
            keys, ray_start_end, pixel_start_end = shdom.gradient.subdivide_raytrace_jobs(rte_sensors, solvers, n_jobs)

            out = Parallel(n_jobs=n_jobs, backend="threading")(
                delayed(gradient_fun, check_pickle=False)(solvers[key],rte_sensors[key].sel(nrays=slice(start,end),npixels=slice(pix_start, pix_end)),
                                                            sensor_mapping[key], forward_sensors,**kwargs)
