...
def lambertian(albedo, ground_temperature=298.15, delx=None, dely=None):
    ...
    if (albedo.size ==1) & (ground_temperature.size ==1):
        ...
        return _make_surface_dataset('variable_lambertian', ground_temperature, delx, dely, albedo=albedo)
    else:
        raise ValueError('ground temperature must have a compatible shape.')


def wave_fresnel(real_refractive_index, imaginary_refractive_index, surface_wind_speed, ground_temperature=298.15,
                delx=None, dely=None):
    """
    TODO
    ...
    if ground_temperature.size == 1:
        ground_temperature = np.full_like(real_refractive_index, fill_value=ground_temperature[0,0])
    else:
        raise ValueError('ground temperature must have a compatible shape')
    ...
    return _make_surface_dataset('wave_fresnel', ground_temperature, delx, dely, real_refractive_index=real_refractive_index,
                                imaginary_refractive_index=imaginary_refractive_index, surface_wind_speed=surface_wind_speed)
...
def diner(A, K, B, ZETA, SIGMA, ground_temperature=298.15, delx=None, dely=None):
    ...
    return _make_surface_dataset('diner', ground_temperature, delx, dely, A=A,K=K,B=B,ZETA=ZETA,SIGMA=SIGMA)


def ocean_unpolarized(surface_wind_speed, pigmentation, ground_temperature=298.15, delx=None, dely=None):
    """
    TODO
    """
    ground_temperature = np.atleast_2d(ground_temperature)
    ...
    return _make_surface_dataset('ocean_unpolarized', ground_temperature, delx, dely, surface_wind_speed=surface_wind_speed,
...
def RPV_unpolarized(RHO0, K, THETA, ground_temperature=298.15, delx=None, dely=None):
    ...
    return _make_surface_dataset('rpv_unpolarized', ground_temperature, delx, dely, RHO0=RHO0,K=K,THETA=THETA)
...
def _make_surface_dataset(surface_type, ground_temperature, delx, dely, **kwargs):
    ...
    list_of_params = [ground_temperature] + [val for val in kwargs.values()]
