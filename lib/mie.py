...
Python binding are implemented for the native SHDOM Mie module for computations of monodisperse, i.e., single particle
scattering properties. Supported particle types are 'water', 'aerosol' (in which case the index of refraction is
user specified) or 'ice' (in which case index of refraction depends on wavelength).
The module also has additional support for integration over arbitrary particle distributions.

For water or ice particles the scattering properties may be averaged over the desired spectral range with
Planck function weighting. The phase functions in the output scattering table are represented with Legendre series.
For polarized output, the six unique elements of the phase matrix are represented with Wigner d-function expansions
...
def get_mono_table(particle_type, wavelength_band, minimum_effective_radius=4.0,
    ...
    -------
    table: xr.Dataset
        A Dataset containing tabulated Legendre coefficients per radius per wavelength. Each entry has 6 six Wigner
        d-function elements of the Mie phase matrix indexed by: P11, P22, P33, P44, P12, P34.
...
def _compute_table(particle_type, wavelength_band,
    ...
    """
    This function does the hard work of computing a monomodal mie table. It has python binding for the SHDOM
    fortran code contained within src/polarized/make_mie_table.f90. See 'get_mono_table' for more details.
...
def _load_table(relative_dir, particle_type, wavelength_band,
    ...
    table: xr.Dataset or None
        If the table exists within the specified path it is loaded and returned. Alternatively, returns None.
    """
    table = None
...
def get_poly_table(size_distribution, mie_mono_table):
    ...
    This function integrates the Mie table over radii for each entry in the size distribution Dataset. This dataset
    could be parameterized arbitrarily, however, a common use-case is according to effective radius and variace.

    Parameters
    ----------
    size_distribution: xr.Dataset
        A Dataset of number_density variable as a function of radius and the table parameterization. The shape of the
        dataset is ('radius', 'param1', 'param2',...'paramN'). A common case is ('radius', 'reff', 'veff').
    mie_mono_table: xr.Dataset
        A Dataset of Mie legendre coefficients as a function of radius. See mie.get_mono_table function
        for more details.

    Returns
    -------
    poly_table: xr.Dataset
        A Dataset with the polydisperse Mie effective scattering properties: extinction, ssalb, legcoef.
        Each of these are a function of the parameterization defined by the size_distribution.
