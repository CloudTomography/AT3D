...
class IndependentNoise:
    ...
    def add_noise(self, sensor, nstokes):
        ...
        sensor['I'] += np.random.normal(size=sensor.sizes['npixels'])* \
                        (sensor['uncertainties'][0, 0].data**(-0.5))
    ...
    def calculate_uncertainties(self, sensor, nstokes):
        ...
        condition = np.where(uncertainty_data > 1e-9)
        uncertainty_data[condition] = 1.0/uncertainty_data[condition]
