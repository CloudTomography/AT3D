...
C           is down to the desired level.
     ...
     .              CURSPLITACC .GT. SPLITACC .AND. .NOT. OUTOFMEM)) )
        ITER = ITER + 1
        PRINT *, ITER
        PRINT *, MAXIV, MAXVAL(RSHPTR(:NPTS+2)), MAXVAL(SHPTR(:NPTS+1))
        PRINT *, MAXVAL(GRIDPTR(:,:NPTS)), MAXIG
...
C           Find the maximum splitting criterion over all processors
        ...
        ENDIF
        PRINT *, 'HELLO RADIANCE_TRUNCATION'
...
C     by MAXIG, MAXIC, MAXIV, MAXIDO) then the OUTOFMEM flag is returned true.
      ...
      PRINT *, 'HELLO SPLIT_GRID', NCELLS, NPTS, DOSPLIT
      IF (DOSPLIT) THEN
        OLDNPTS = NPTS
