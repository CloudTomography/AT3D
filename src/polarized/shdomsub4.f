...
Cf2py intent(in) :: COSTFUNC
      ...
      DOUBLE PRECISION GRADOUT(NBPTS,NUMDER,NGRAD)
...
C           These are also the points we need the gradient for.
        DO I = 1, 8
...
C                 Linear extinction, linear source*extinction, to second order
     ...
     .                *(1.0 - 0.05*(EXT1-EXT0)*DELS) )/EXT
            IF (.NOT. OUTOFDOMAIN) THEN
C             Integrating the 'source of the linearized RTE' weighted by transmission
...
C     and base grid points.
      DO N = 1, 8
C     Note - the outer loop should be NUMDER/NPART not N for efficiency.
        IP = GRIDPTR(N,ICELL)
        I = DONETHIS(N)
...
C                   grid point.
     ...
     .                + FULL_SINGSCAT(:)
                ...
                IF (SRCTYPE .NE. 'S') THEN
                  GRAD8(:,NB,N,IDR) = GRAD8(:,NB,N,IDR) +
     .              DEXTM*PLANCK(IP, IPA) - EXTINCT(IP,IPA)*
