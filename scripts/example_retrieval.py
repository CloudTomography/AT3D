...
#in that case.
sensor_dict, solvers, rte_grid =pyshdom.util.load_forward_model(file_name)

#define an approximate forward_sensor that doesn't have any subpixel rays.
# forward_sensors = pyshdom.util.SensorsDict()
# for instrument in sensor_dict:
#     sensor_list = sensor_dict[instrument]['sensor_list']
#     for sensor in sensor_list:
#         approximated_sensor = pyshdom.sensor.make_sensor_dataset(sensor.cam_x,
#                                                                sensor.cam_y,
#                                                                sensor.cam_z,
#                                                                sensor.cam_mu,
#                                                                sensor.cam_phi,
...
#make an rte_grid
# rte_grid = pyshdom.grid.make_grid(0.0, _.x.data.max(), 25, 0.0, _.y.data.max(), 25,
...
initial_on_grid = pyshdom.grid.resample_onto_grid(rte_grid, initial)
...
def set_state_fn(state):
    ...
    for wavelength in wavelengths:
        ...
        solvers_reconstruct.add_solver( wavelength,
                                pyshdom.solver.RTE(numerical_params=config,
...
def project_gradient_to_state(gradient):
    #for help writing this run the gradient e.g. pyshdom.gradient.levis_approx_uncorrelated_l2
...
obj_fun = pyshdom.optimize.ObjectiveFunction.LevisApproxUncorrelatedL2(sensor_dict, solvers_reconstruct,
...
optimizer = pyshdom.optimize.Optimizer(obj_fun)
...
pyshdom.util.save_forward_model(save_name, forward_sensors, solvers_reconstruct)
