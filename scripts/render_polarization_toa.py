...
exitting stokes vector at the top of the the domain. This sensor is an approximation (somewhat crude) for far observing satellites where the rays are parallel.
...
For a tutorial overview of how to operate the forward rendering model see the following notebooks:
 ...
 - notebooks/Rayleigh Polarization Rendering.ipynb
 - notebooks/Mie Polarization Rendering.ipynb
"""
...
def argument_parsing():
    ...
    parser.add_argument('--mie_base_path',
                        default='mie_tables/polydisperse/Water_<wavelength>nm.scatpol',
                        help='(default value: %(default)s) Polarized Mie table base file name. ' \
...
def solve_rte(atmospheres):
    ...
    for wavelength, solar_flux in zip(args.wavelength, solar_fluxes):
        ...
        rte_solver = shdom.RteSolver(scene_params, numerical_params, num_stokes=args.num_stokes)
