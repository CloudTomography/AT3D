...
if __name__ == "__main__":
    ...
    # Two options - either root creates the atmosphere and broadcasts it or 
    # it is generated by each individual node. Probably little difference.
    
    if comm_rank == 0:
      #atmospheres = generate_atmosphere()
      rte_solvers = shdom.RteSolverArray()
      
    #atmospheres = comm.bcast(atmospheres, root = 0)
    atmospheres = generate_atmosphere()

    for i, wavelength, solar_flux in enumerate(zip(arg.wavelength,arg.solar_flux)):
    ...
    rte_solver_list = comm.gather(rte_solver, root = 0)
    #measurement_list = comm.gather(measurements, root = 0)
    #images = np.concatenate([measurement.images for images in measurement_list])
    
    for solver in rte_solver_list:
