...
rte_grid = pyshdom.grid.make_grid(cloud_scatterer.x.data.min(),cloud_scatterer.x.data.max(),cloud_scatterer.x.data.size,
...
for zenith,azimuth in zip(sensor_zenith_list,sensor_azimuth_list):
    misr_list.append(
                    pyshdom.sensor.orthographic_projection(0.86, cloud_scatterer,0.02,0.02, azimuth, zenith,
                                             altitude='TOA', stokes='I')#, sub_pixel_ray_args={'method': pyshdom.sensor.gaussian,
...
wavelengths = pyshdom.script_util.get_unique_wavelengths(Sensordict)
...
for wavelength in wavelengths:
    ...
    surfaces[wavelength] = pyshdom.surface.lambertian(albedo=0.0) #surface is wavelength independent.
    sources[wavelength] = pyshdom.source.solar(-1*np.cos(np.deg2rad(10.0)),0.0,solarflux=1.0)
    ...
    cloud_optical_scatterers[wavelength] = pyshdom.medium.table_to_grid(cloud_scatterer_on_rte_grid, poly_table)
...
mediums = pyshdom.script_util.combine_to_medium({'cloud':cloud_optical_scatterers, 'rayleigh':rayleigh_scatterer_list})

#define all solvers.
solvers = OrderedDict()
for key,name in names.items():
    solvers[key] = pyshdom.solver.RTE(numerical_params=numerical_parameters[key],
                                    medium=mediums[key],
                                   source=sources[key],
...
pyshdom.script_util.get_measurements(solvers, Sensordict, maxiter=100, n_jobs=8, verbose=True)
shutil.copy(__file__, 'copied_script.py')
#save measurements, and also document all inputs.
pyshdom.util.save_forward_model(file_name, Sensordict, solvers)
